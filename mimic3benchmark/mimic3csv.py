...
def read_events_table_by_row(mimic3_path, table):
    file_path = os.path.join(mimic3_path, table + ".csv")
    with open(file_path, "r") as f:
        # Count lines excluding the header
        num_rows = sum(1 for _ in f) - 1
    reader = csv.DictReader(open(os.path.join(mimic3_path, table + ".csv"), "r"))
    for i, row in enumerate(reader):
        if "icustay_id" not in row:
            row["icustay_id"] = ""
        yield row, i, num_rows


def count_icd_codes(diagnoses, output_path=None):
    codes = (
        diagnoses[["icd9_code", "short_title", "long_title"]]
        .drop_duplicates()
        .set_index("icd9_code")
    )
...
def add_age_to_icustays(stays):
    stays["age"] = stays.apply(
        lambda e: (
            e["intime"].to_pydatetime() - e["dob"].to_pydatetime()
        ).total_seconds()
        / 3600.0
        / 24.0
        / 365.0,
...
def read_events_table_and_break_up_by_subject(
    ...
    with open(os.path.join(mimic3_path, table + ".csv"), "r") as f:
        nb_rows = sum(1 for _ in f) - 1

    for row, row_no, _ in tqdm(
        read_events_table_by_row(mimic3_path, table),
        total=nb_rows,
