...
def main():
    ...
    icustay_missing_in_stays = (
        0  # ICUSTAY_ID does not appear in stays.csv. We exclude such events.
    )
    ...
    for subject in tqdm(subjects, desc="Iterating over subjects"):
        ...
        to_write.to_csv(
            os.path.join(args.subjects_root_path, subject, "events.csv"), index=False
        )
